<style>
    .fade-up {
      opacity: 0;
      transform: translateY(20px);
      transition: opacity 0.8s ease-out, transform 0.8s ease-out;
    }
  
    .in-view {
      opacity: 1;
      transform: translateY(0);
    }
  </style>

<script>
    document.addEventListener("DOMContentLoaded", function () {
      const elements = document.querySelectorAll(".fade-up");
  
      const observer = new IntersectionObserver(
        (entries) => {
          entries.forEach((entry) => {
            if (entry.isIntersecting) {
              entry.target.classList.add("in-view");
            } 
          });
        },
        { threshold: 0.3 }
      );
  
      elements.forEach((el) => observer.observe(el));
    });
  </script>

<div class="mt-28 flex flex-col items-center">
    {{-- Header --}}
    <div class="w-full flex flex-wrap lg:flex-nowrap gap-10 p-10">
        <div class="w-full lg:w-1/2 flex justify-center lg:justify-end p-3">
            <div class="w-60 h-60 rounded-full overflow-hidden shadow-lg">
                <img class="w-full h-full object-cover shadow-md" src="{{ asset('img/Asian girl on blue 500px.jpeg') }}" alt="">
            </div>
        </div>
        <div class="w-full lg:w-1/2 flex flex-col gap-2 text-center lg:text-left">
            <h1 class="font-semibold text-2xl md:text-3xl w-full lg:w-2/3">Harga Murah, Pengerjaan Terjamin & Terpercaya!</h1>
            <p class="font-poppins w-full lg:w-2/3">Sheets Si Teman Tugasmu adalah solusi dalam menyelesaikan tugas sekolah. </p>
            <div class="flex flex-col gap-2 items-center lg:items-start">
                <h1 class="bg-blue-500 text-white py-2 px-6 w-6/12 lg:w-3/12 text-center rounded-full">Layanan Kami:</h1>
                <div class="">
                    <div class="flex gap-2 text-sm items-center w-52 md:justify-start justify-center">
                        <div class="w-2/3 flex gap-2">
                            <i class="bi bi-check-circle-fill text-blue-700"></i>
                            <p>Konsultasi gratis</p>
                        </div>
                    </div>
                    <div class="flex gap-2 text-sm items-center md:justify-start justify-center w-52">
                        <div class="w-2/3 flex gap-2 ">
                            <i class="bi bi-check-circle-fill text-blue-700"></i>
                            <p>Privasi gratis</p>
                        </div>
                    </div>
                    <div class="flex gap-2 text-sm items-center md:justify-start justify-center w-52">
                        <div class="w-2/3 flex gap-2">
                            <i class="bi bi-check-circle-fill text-blue-700"></i>
                            <p>Revisi gratis</p>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>

    {{-- rectangle biru --}}
    <div class="w-full bg-blue-500 h-3 shadow-lg"></div>
    {{-- wave --}}
    <img class="h-auto object-cover w-full" src="{{ asset('svg/wave-kebalik3.svg') }}" alt="">

    {{-- Tugas Apapun Bisa, Harga murah cuy, Berkualitas & terpercaya --}}
    <div class="fade-up flex flex-wrap md:flex-nowrap justify-evenly w-3/4 md:w-3/4 gap-6 mt-6">
        <!-- Card 1 -->
        <div class="flex flex-col items-center justify-center space-y-2 w-full md:w-1/3 hover:scale-110 transition-all">
            <div style="font-size: 60px">
                <i class="bi bi-journals"></i>
            </div>
            <p class="text-center font-semibold text-md">Tugas apapun bisa</p>
        </div>

        <!-- Card 2 -->
        <div class="flex flex-col items-center justify-center space-y-2 w-full md:w-1/3 hover:scale-110 transition-all">
            <div style="font-size: 60px">
                <i class="bi bi-cash-coin"></i>
            </div>
            <p class="text-center font-semibold text-md">Harga murah</p>
        </div>

        <!-- Card 3 -->
        <div class="flex flex-col items-center justify-center space-y-2 w-full md:w-1/3 hover:scale-110 transition-all">
            <div style="font-size: 60px;">
                <i class="bi bi-hand-thumbs-up"></i>
            </div>
            <p class="text-center font-semibold text-md">Berkualitas & Terpercaya</p>
        </div>
    </div>

    {{-- Tugas yang bisa kami kerjakan --}}
    <div class="fade-up mt-28 flex flex-col items-center w-full md:w-3/4">
        <h2 class="font-poppins flex justify-center font-bold text-xs md:text-xl">Tugas yang bisa kami kerjakan</h2>
        <div class="flex flex-col md:flex-row items-center justify-center gap-6 py-4 w-4/5 md:w-full p-4 ">
            <div class="w-10/12 md:w-1/3 flex flex-col items-center gap-6">
                <div class="w-full h-60 overflow-hidden">
                    <img class="rounded-md object-cover w-full h-full" src="{{ asset('img/akson-1K8pIbIrhkQ-unsplash.jpg') }}" alt="">
                </div> 
                <p class="font-poppins text-sm text-center">Tugas Kelompok</p>
            </div>
            <div class="w-10/12 md:w-1/3 flex flex-col items-center gap-6">
                <div class="w-full h-60 overflow-hidden">
                    <img class="rounded-md object-cover w-full h-full" src="{{ asset('img/andrew-neel-wClUbRMCBD8-unsplash.jpg') }}" alt="">
                </div> 
                <p class="font-poppins text-sm text-center">Tugas Individu</p>
            </div>
            <div class="w-10/12 md:w-1/3 flex flex-col items-center gap-6">
                <div class="w-full h-60 overflow-hidden">
                    <img class="rounded-md object-cover w-full h-full" src="{{ asset('img/herlambang-tinasih-gusti-nxXBUhe17L4-unsplash.jpg') }}" alt="">
                </div> 
                <p class="font-poppins text-sm text-center">Tugas PPT & Makalah</p>
            </div>
        </div>
    </div>

    {{-- Tugas telah dikerjakan --}}
    <div class="mt-10 fade-up flex flex-col md:flex-row gap-8 md:gap-10 bg-blue-500 w-2/3 md:w-2/3 p-5 rounded-md justify-evenly text-white">
        <div class="flex flex-col items-center">
            <i class="bi bi-mortarboard text-4xl"></i>
            <p class="text-xl font-bold">122</p>
            <p class="text-xl font-bold">Pengguna</p>
        </div>
        <div class="flex flex-col items-center">
            <i class="bi bi-book text-4xl"></i>
            <p class="text-xl font-bold">122</p>
            <p class="text-xl font-bold">Tugas Di Kerjakan</p>
        </div>
        <div class="flex flex-col items-center">
            <i class="bi bi-people text-4xl"></i>
            <p class="text-xl font-bold">12</p>
            <p class="text-xl font-bold">Tim profesional</p>
        </div>
    </div>

    {{-- Jenjang joki tugas yang kami kerjakan --}}
    <div class="mt-10 fade-up flex items-center flex-col w-5/6 md:w-4/5">
        <h2 class="font-bold text-xs md:text-xl font-poppins w-1/2">Jenjang Sheets Si Teman Tugas Yang Dikerjakan</h2>
        {{-- container card jenjang sheets --}}
        <div class="flex flex-col md:flex-row gap-4 md:w-full p-10">
            <!-- Card 1 -->
            <div class="w-full md:w-1/3 flex flex-col items-center gap-6">
                <div class="w-full rounded-xl border shadow-xl relative overflow-hidden group">
                    <div class="w-full h-96 relative">
                        <img class="object-cover w-full h-full transform transition-transform duration-500 group-hover:scale-105" src="{{ asset('img/anak sd.jpg') }}" alt="">
                        <div class="absolute inset-0 bg-black bg-opacity-50 opacity-0 transition-opacity duration-500 group-hover:opacity-80"></div>
                        <div class="absolute bottom-5 left-1/2 transform -translate-x-1/2 opacity-0 translate-y-5 transition-all duration-500 group-hover:opacity-100 group-hover:translate-y-0 w-4/5 h-2/3 bg-white p-5 rounded-lg shadow-lg text-center">
                            <p class="text-black text-lg font-bold font-poppins mb-7">TUGAS SEKOLAH SD</p>
                            <p class="text-black text-lg font-bold font-poppins mb-0">Harga Tugas Sekolah SD</p>
                            <p class="text-blue-500 hover:text-black transition-all text-lg font-semibold mb-4">Rp10.000 - 20.000</p>
                            <a href="{{ route('payment.form') }}" class="flex items-center font-semibold rounded-full mt-10 px-4 py-2 transition-all duration-300 group/button relative overflow-hidden">
                                <div class="w-10 h-10 bg-blue-700 rounded-full flex items-center justify-center">
                                    <i class="bi bi-cart-fill text-white text-xl"></i>
                                </div>
                                <div class="absolute left-12 opacity-0 flex justify-center w-1/2 h-3/4 bg-blue-500 rounded-md overflow-hidden items-center transition-all hover:bg-blue-800 duration-300 group-hover/button:opacity-100 group-hover/button:left-16">
                                    <span class="text-white font-poppins">PESAN</span>
                                </div>
                            </a>
                        </div>
                    </div>
                </div>
            </div>
        
            <!-- Card 2 -->
            <div class="w-full md:w-1/3 flex flex-col items-center gap-6">
                <div class="w-full rounded-xl border shadow-xl relative overflow-hidden group">
                    <div class="w-full h-96 relative">
                        <img class="object-cover w-full h-full transform transition-transform duration-500 group-hover:scale-105" 
                            src="{{ asset('img/IMG_2895-scaled.jpg') }}" 
                            alt="">
                        <div class="absolute inset-0 bg-black bg-opacity-50 opacity-0 transition-opacity duration-500 group-hover:opacity-80"></div>
                        <div class="absolute bottom-5 left-1/2 transform -translate-x-1/2 opacity-0 translate-y-5 transition-all duration-500 group-hover:opacity-100 group-hover:translate-y-0 w-4/5 h-2/3 bg-white p-5 rounded-lg shadow-lg text-center">
                            <p class="text-black text-lg font-semibold mb-4">TUGAS SEKOLAH SMP</p>
                            <p class="text-black text-lg font-bold font-poppins mb-0">Harga Tugas Sekolah SMP</p>
                            <p class="text-blue-500 hover:text-black transition-all text-lg font-semibold mb-4">Rp20.000 - 35.000</p>
                            <a href="{{ route('payment.form') }}" class="flex items-center font-semibold rounded-full mt-10 px-4 py-2 transition-all duration-300 group/button relative overflow-hidden">
                                <div class="w-10 h-10 bg-blue-700 rounded-full flex items-center justify-center">
                                    <i class="bi bi-cart-fill text-white text-xl"></i>
                                </div>
                                <div class="absolute left-12 opacity-0 flex justify-center w-1/2 h-3/4 bg-blue-500 rounded-md overflow-hidden items-center transition-all hover:bg-blue-800 duration-300 group-hover/button:opacity-100 group-hover/button:left-16">
                                    <span class="text-white font-poppins">PESAN</span>
                                </div>
                            </a>
                        </div>
                    </div>
                </div>
            </div>
        
            <!-- Card 3 -->
            <div class="w-full md:w-1/3 flex flex-col items-center gap-6">
                <div class="w-full rounded-xl border shadow-xl relative overflow-hidden group">
                    <div class="w-full h-96 relative">
                        <img class="object-cover w-full h-full transform transition-transform duration-500 group-hover:scale-105" 
                            src="{{ asset('img/anak sma.png') }}" 
                            alt="">
                        <div class="absolute inset-0 bg-black bg-opacity-50 opacity-0 transition-opacity duration-500 group-hover:opacity-80"></div>
                        <div class="absolute bottom-5 left-1/2 transform -translate-x-1/2 opacity-0 translate-y-5 transition-all duration-500 group-hover:opacity-100 group-hover:translate-y-0 w-4/5 h-2/3 bg-white p-5 rounded-lg shadow-lg text-center">
                            <p class="text-black text-lg font-semibold mb-4">TUGAS SEKOLAH SMA</p>
                            <p class="text-black text-lg font-bold font-poppins mb-0">Harga Tugas Sekolah SMA</p>
                            <p class="text-blue-500 hover:text-black transition-all text-lg font-semibold mb-4">Rp45.000 - 60.000</p>
                            <a href="{{ route('payment.form') }}" class="flex items-center font-semibold rounded-full mt-10 px-4 py-2 transition-all duration-300 group/button relative overflow-hidden">
                                <div class="w-10 h-10 bg-blue-700 rounded-full flex items-center justify-center">
                                    <i class="bi bi-cart-fill text-white text-xl"></i>
                                </div>
                                <div class="absolute left-12 opacity-0 flex justify-center w-1/2 h-3/4 bg-blue-500 rounded-md overflow-hidden items-center transition-all hover:bg-blue-800 duration-300 group-hover/button:opacity-100 group-hover/button:left-16">
                                    <span class="text-white font-poppins">PESAN</span>
                                </div>
                            </a>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>

    {{-- JOki tugas murah disini --}}
    <div class="mx-auto fade-up py-10 space-y-8 w-3/4 md:w-3/4">
        <!-- Section 1 -->
        <div class="flex flex-col md:flex-row items-center bg-white shadow-md rounded-lg overflow-hidden">
            <img src="{{ asset('img/ed-us-RwZzAcRmbbI-unsplash (1).jpg') }}" alt="Joki Murah" class="w-full md:w-1/3 object-cover rounded-br-xl rounded-bl-xl rounded-tr-xl rounded-tl-xl  shadow-lg">
            <div class="p-6">
                <h2 class="text-2xl font-bold text-blue-600 mb-4">MENYELESAIKAN TUGAS DISINI MURAH</h2>
                <p class="text-gray-700 leading-relaxed">
                    Kamu yang mempunyai budget kurang? Atau pas-pasan? Tenang aja di sini harganya murah!
                </p>
            </div>
        </div>

        <!-- Section 2 -->
        <div class="flex flex-col md:flex-row items-center bg-white shadow-md rounded-lg overflow-hidden">
            <img src="{{ asset('img/assad-tanoli-DreH1YbP1js-unsplash.jpg') }}" alt="Berkualitas dan Terpercaya" class="w-full md:w-1/3 object-cover rounded-br-xl rounded-bl-xl rounded-tr-xl rounded-tl-xl shadow-lg">
            <div class="p-6">
                <h2 class="text-2xl font-bold text-blue-600 mb-4">BERKUALITAS DAN TERPERCAYA</h2>
                <p class="text-gray-700 leading-relaxed">
                    Takut tugasnya nilainya jelek? Atau takut selesainya lama? Tenang aja di sini kita punya tim profesional 
                    yang berkualitas dan pasti tugas kamu akan dikerjakan secepat mungkin!
                </p>
            </div>
        </div>

        <!-- Section 3 -->
        <div class="flex flex-col md:flex-row items-center bg-white shadow-md rounded-lg overflow-hidden">
            <img src="{{ asset('img/Premium Photo _ Three asian girls in veils in school uniforms stand smiling using a laptop together while carrying b___.png') }}" alt="Revisi Gratis" class="w-full md:w-1/3 object-cover">
            <div class="p-6">
                <h2 class="text-2xl font-bold text-blue-600 mb-4">REVISI GRATIS</h2>
                <p class="text-gray-700 leading-relaxed">
                    Gimana? tugasnya ditolak sama guru? Atau kalian gak puas sama hasilnya? Santai... di sini kalian bisa revisi tugas 
                    kalian secara gratis.
                </p>
            </div>
        </div>
    </div>
</div>